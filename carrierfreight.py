...
class CarrierFreight:
    ...
    def __init__(self, main_graph_str, input_paths):
        ...
        #xsls paths
        self.input_path_file_xslx = input_paths["input_path_file_xslx"] + input_paths["input_file_directory"]
        #rates
    ...
    def prepare_data(self):
        ...
        # trips
        df = pd.DataFrame(columns=['Fecha', 'Matrícula', 'Peso', 'Origen', 'Destino', 'Remito', 'Pago a'])
        for bdu_name in list_bdu_names:
            ...
            bdu_read.columns = ['Fecha', 'Matrícula', 'Peso', 'Origen', 'Destino', 'Remito', 'Transportista']
            bdu_read = bdu_read[['Fecha', 'Matrícula', 'Peso', 'Origen', 'Destino', 'Remito', 'Transportista']]
            ...
            logger.info("{value} rows have been added to the travel dataframe".format(value=bdu_read.shape[0]))
        df["Remito"] = df["Remito"].apply(str)
        self.bdu = df

        # fuel
        if os.path.isfile(self.input_path_file_xslx + self.input_path_fuel):
            logger.info("processing fuel form {fuel_sheet_name}".format(fuel_sheet_name=self.input_path_fuel))
            df_fuel_keys = pd.read_excel(self.input_path_file_xslx + self.input_path_fuel, sheet_name=None)
            sheet_name_list = []
            for sheet_name in df_fuel_keys.keys():
                sheet_name_list.append(sheet_name)
        
            df_fuel_concat = pd.DataFrame(columns=['Fecha de Emisión', 'Matrícula', 'Comprobante', 'Debe', 'Transportista', 'Estacion'])
            for sheet_name in sheet_name_list:
                df_fuel_read = pd.read_excel(self.input_path_file_xslx + self.input_path_fuel, sheet_name=sheet_name, usecols="A,B,C,D,E,F")
                df_fuel_read = df_fuel_read[['Fecha de Emisión', 'Matrícula', 'Comprobante', 'Debe', 'Transportista', 'Estacion']]
                df_fuel_read["Matrícula"].fillna("", inplace = True)
                df_fuel_concat = pd.concat([df_fuel_concat, df_fuel_read], ignore_index=True)
                logger.info("{value} rows have been added to the fuel dataframe from {carrier}".format(value=df_fuel_read.shape[0], carrier=sheet_name))
            self.df_fuel = df_fuel_concat
            logger.info("total number of rows of the fuel dataframe is {value}".format(value=self.df_fuel.shape[0]))

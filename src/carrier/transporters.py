...
def get_truck_id_with_rates(df_bdu_with_rates):
    ...
    df_truck_id_with_rates = pd.merge(df_truck_id, df_bdu_with_rates, how='right', left_on=["Matrícula"], right_on=["Matrícula"])
    df_truck_id_with_rates = df_truck_id_with_rates.sort_values(by=['Remito'])
    df_truck_id_with_rates = df_truck_id_with_rates[["Fecha", "Matrícula", "Peso", "Tarifa_Sub", "Total", "Origen", "Destino", "Remito", "Pago a", "Transportista"]]
    return df_truck_id_with_rates

def get_transporters_rut():
    path_file_dinama = get_input_path_file_xslx() + get_dinama_file_name()
    sheet_name = get_dinama_sheet_name()
    df_transporters_rut = read_data_frame(path_file_dinama, sheet=sheet_name, use_cols="B,C,D,E,F,G")
    df_transporters_rut.columns = ['Transportista','Rut','BANCO','TIPO CUENTA','CUENTA','ANTERIOR']

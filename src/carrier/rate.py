...
def get_rate():
    return read_data_frame(get_input_path_rate(), sheet="Actual", use_cols="A,B,D")
...
def get_bdus_with_rates(bdu_df):
    rate_df = get_rate()
    df_rate = pd.merge(bdu_df, rate_df[['Sector/Pila origen', 'Destino', 'Tarifa_Sub']], how='left',left_on=['Origen','Destino'], right_on=['Sector/Pila origen','Destino'])
...
def get_tree_field_name(source, destination):
    ...
    return df_rate[(df_rate.Sector_pila_origen==source) & (df_rate.Destino==destination)].nombre_detalle.iloc[0]

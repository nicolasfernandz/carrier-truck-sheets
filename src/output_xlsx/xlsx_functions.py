...
def write_header(writer, sheetName, index_row):
    if contractor == "EDI":
        ...
        df_header.to_excel(writer, sheet_name=sheetName, index=False, startrow=index_row, startcol=0)
...
def write_carrier(writer, sheetName, index_row, df_carrier_rut):
    data = {'Etiqueta':['Fletero', 'Rut'], 'Valor':[df_carrier_rut.iloc[0, 0], df_carrier_rut.iloc[0, 1]]}
...
def write_total_exempt(writer, sheetName, index_row, workbook, worksheet, total_excempt):
    data = {'Etiqueta':['TOTAL FACTURA EXENTA - (Asimilado a exportacion ART. 34 DEC 220/998)'],'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[total_excempt]}
    df_total_exempt = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_total_exempt.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
...
def write_sub_total_tax(writer, sheetName, index_row, workbook, worksheet, total_tax):
    data = {'Etiqueta':['SUBTOTAL FACTURA CON IVA'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[total_tax]}
    df_total_tax = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_total_tax.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
    index_row += 1
...
def write_tax(writer, sheetName, index_row, workbook, worksheet, tax):
    ...
    df_tax.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
    index_row += 1
...
def write_total_tax(writer, sheetName, index_row, workbook, worksheet, total_bill_tax):
    data = {'Etiqueta':['TOTAL FACTURA CON IVA'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[total_bill_tax]}
    df_total_bill_tax = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_total_bill_tax.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
...
def write_retention_tax(writer, sheetName, index_row, workbook, worksheet, retention_tax):
    data = {'Etiqueta':['RETENCION DE IVA'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[retention_tax]}
    df_retention_tax = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_retention_tax.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
    index_row += 1
...
def write_credit_certificate(writer, sheetName, index_row, workbook, worksheet, credit_certificate):
    data = {'Etiqueta':['CERTIFICADO DE CREDITO'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[credit_certificate]}
    df_credit_certificate = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_credit_certificate.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
...
def write_total_fuel_consumption(writer, sheetName, index_row, workbook, worksheet, total_to_be_deducted):
    data = {'Etiqueta':['DESCUENTO COMBUSTIBLE'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[total_to_be_deducted]}
    df_fuel_discount = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_fuel_discount.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
    index_row += 1
...
def write_total_to_be_charged(writer, sheetName, index_row, workbook, worksheet, total_to_be_charged):
    data = {'Etiqueta':['TOTAL A COBRAR'], 'B':['B'], 'C':['C'], 'D':['D'], 'Valor':[total_to_be_charged]}
    df_total_to_be_charged = pd.DataFrame (data, columns = ['Etiqueta', 'B', 'C', 'D', 'Valor'])
    df_total_to_be_charged.to_excel(writer, sheet_name=sheetName, header=False, index=False, startrow=index_row, startcol=0)
    index_row += 1
...
def write_exempt_forests(vec_forest, df_carrier, writer, sheetName, index_row):
    ...
    for i in range(0, len(vec_forest)):
        ...
        df_carrier_forest=df_carrier_forest.sort_values(by=['Fecha llegada en balanza'])
        df_carrier_forest["Fecha llegada en balanza"] = pd.to_datetime(df_carrier_forest["Fecha llegada en balanza"]).dt.strftime("%d/%m/%Y")
        ...
        bdu_total = pd.concat([df_carrier_forest, df_total_forest.to_frame().T], ignore_index=True)
        ...
        worksheet.write(index_row-2, 0, get_tree_field_name(forest, _destination), headers.get_cell_format(workbook, top=0, left=0, right=0, bottom=2, liquid_de=contractor))
...
def write_trips(df_truck_id_with_rates, df_transporters_rut, df_fuel, carrier, writer, df_payment_summary):
    # get all trips from the Carrier
    df_carrier = df_truck_id_with_rates[df_truck_id_with_rates['Transportista'] == carrier]
    ...
    if(not df_carrier.empty):
        ...
        if(len(vec_forest) > 0):
            index_row = 0
            df_carrier_rut = df_transporters_rut[df_transporters_rut['Transportista'] == carrier]
            ...
            print(carrier + " " + rut + " " + banco + " " + tipo_cuenta + " " + str(cuenta) + " " + str(anterior))
            ...
            index_row, workbook, worksheet = write_carrier(writer, sheetName, index_row, df_carrier_rut)

            df_carrier = df_carrier[["Fecha", "Matrícula", "Peso", "Tarifa_Sub", "Total", "Origen", "Destino", "Remito"]]
            df_carrier.columns = ["Fecha llegada en balanza", "Camión", "Peso neto", "$/tonelada", "$ Totales", "Origen", "Destino", "Remito"]

            df_carrier_excempt = df_carrier[(df_carrier['Destino'] == "MDP") | (df_carrier['Destino'] == "UPM") | (df_carrier['Destino'] == "UPM2")]

            index_row, total_excempt = write_exempt_forests(vec_forest, df_carrier_excempt, writer, sheetName, index_row)

            # write total exempt if there is travels
            if total_excempt > 0.0:
                index_row = write_total_exempt(writer, sheetName, index_row, workbook, worksheet, total_excempt)

            # TO DO - TAX!!!
            df_carrier_tax = df_carrier[(df_carrier['Destino'] == "Kluntex SA") | (df_carrier['Destino'] == "Murchison S.A.") | (df_carrier['Destino'] == "Sierra Verde") | (df_carrier['Destino'] == "Acopio Olimar") | (df_carrier['Destino'] == "Planta Olimar") | (df_carrier['Destino'] == "ACOPIO CERRO CHATO") | (df_carrier['Destino'] == "ACOPIO") | (df_carrier['Destino'] == "Balanza") | (df_carrier['Destino'] == "PLANIR MVDEO") | (df_carrier['Destino'] == "PLANIR PROGRESO") | (df_carrier['Destino'] == "ACOPIO SANTAMARIA") | (df_carrier['Destino'] == "Acopio Santamaria") | (df_carrier['Destino'] == "ACOPIO PLANIR") | (df_carrier['Destino'] == "ACOPIO TACUAREMBÓ") | (df_carrier['Destino'] == "JUANGO") | (df_carrier['Destino'] == "Chipper") | (df_carrier['Destino'] == "DEPOSITO DE PACO") | (df_carrier['Destino'] == "URCEL") | (df_carrier['Destino'] == "FAS") | (df_carrier['Destino'] == "IDALEN S.A.") | (df_carrier['Destino'] == "BALANZA TROZA 5") | (df_carrier['Destino'] == "Transportes José Pedro Varela S. A")]

            print(df_carrier_tax)

            index_row, total_tax = write_forests_with_tax(vec_forest, df_carrier_tax, writer, sheetName, index_row)

            if total_tax > 0.0:
                index_row = write_sub_total_tax(writer, sheetName, index_row, workbook, worksheet, total_tax)
            else:
            ...
            total_to_be_charged = total_excempt + total_bill_with_tax - retention_tax_iva - credit_certificate

            if total_tax > 0.0:
                index_row = write_tax(writer, sheetName, index_row, workbook, worksheet, tax_iva)
            else:
                index_row = write_tax(writer, sheetName, index_row, workbook, worksheet, "-")

            if total_bill_with_tax > 0.0:
                index_row = write_total_tax(writer, sheetName, index_row, workbook, worksheet, total_bill_with_tax)
            else:
                index_row = write_total_tax(writer, sheetName, index_row, workbook, worksheet, "-")

            if retention_tax_iva > 0.0:
                index_row = write_retention_tax(writer, sheetName, index_row, workbook, worksheet, retention_tax_iva * -1)
            else:
                index_row = write_retention_tax(writer, sheetName, index_row, workbook, worksheet, "-")

            if credit_certificate > 0.0:
                index_row = write_credit_certificate(writer, sheetName, index_row, workbook, worksheet, credit_certificate * -1)
            else:
            ...
            if(is_fuel_present() and df_fuel_loads['Fecha de Emisión'].count() > 0):
                ...
                for i in range(0, len(vec_stations)):
                    ...
                    df_fuel_service_station = df_fuel_loads[df_fuel_loads["Estacion"] == fuel_service_station]
                    df_fuel_service_station = df_fuel_service_station[['Fecha de Emisión', 'Camión', 'Comprobante', 'Importe']]
                    ...
                    df_fuel_with_total = pd.concat([df_fuel_service_station, df_total_load.to_frame().T], ignore_index=True)
                    df_fuel_with_total.to_excel(writer, sheet_name=sheetName, index=False, startrow=index_row, startcol=0)
                    ...
                    worksheet.write(index_row-2, 0, 'Total Combustible en ' + fuel_service_station, headers.get_cell_format(workbook, top=0, left=0, right=0, bottom=2, liquid_de=contractor))
                ...
                if total_fuel > 0.0:
                    index_row_fuel_discount_or_total_to_be_charged = write_total_fuel_consumption(writer,
                                                                                                sheetName,
                                                                                                index_row_fuel_discount_or_total_to_be_charged,
                                                                                                workbook, worksheet, total_fuel * -1)

            if total_fuel > 0.0:
                write_total_to_be_charged(writer, sheetName, index_row_fuel_discount_or_total_to_be_charged + 1 , workbook, worksheet, total_to_be_charged - total_fuel)
            else:
                write_total_to_be_charged(writer, sheetName, index_row_fuel_discount_or_total_to_be_charged, workbook, worksheet, total_to_be_charged)

        data = {'EMPRESA':  [carrier], 'BANCO':[banco], 'TIPO CUENTA':[tipo_cuenta], 'CUENTA':[cuenta], 'ANTERIOR':[anterior], 'IMPORTE A GIRAR':[total_to_be_charged - total_fuel], 'RUT':[rut], 'RETENCION':[retention_tax_iva], 'CERT':[credit_certificate], 'DESC. COMBUSTIBLE':[total_fuel] }
        if df_payment_summary is None:
            df_payment_summary = pd.DataFrame (data, columns = ['EMPRESA', 'BANCO', 'TIPO CUENTA', 'CUENTA', 'ANTERIOR', 'IMPORTE A GIRAR', 'RUT', 'RETENCION', 'CERT', 'DESC. COMBUSTIBLE'])
        else:
            ...
            df_payment_summary = pd.concat([df_payment_summary, df_new_resumen_pago], ignore_index=True)
